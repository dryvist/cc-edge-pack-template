...
describe('route structure', () => {
  it('route.yml exists', () => {
    expect(existsSync(ROUTE_YML)).toBe(true);
  });
  ...
  it('routes use __group output (vct-cribl-pack-validator rule)', () => {
    const config = loadRoutes();
    const bad = (config.routes ?? []).filter((r) => r.output !== '__group').map((r) => r.id);
    expect(
      bad.length,
...
describe('route flow (dynamic)', () => {
  ...
  const routesForFlow = (config.routes ?? []).filter((r) => r.disabled !== true);

  if (routesForFlow.length === 0) {
    it.skip('no routes declared in default/pipelines/route.yml', () => undefined);
    return;
  }
  ...
  for (const route of routesForFlow) {
    ...
    it.skipIf(skipReason !== null)(
      ...
      async () => {
        ...
        const sampleId = await client.saveSample(`route-flow-${route.id}`, [event]);
        try {
          const result = await client.runRouteFlow(sampleId, [event], { pack: packId });
          expect(
            result.route.id,
